...
def collect_and_download(derivative, pipeline, strategy, out_dir, diagnosis):
    ...
    # Init variables
    mean_fd_thresh = 0.2  # func_mean_fd
    ...
    for pheno_row in pheno_list[1:]:
        ...
        s3_path = '/'.join([s3_prefix, 'Outputs', pipeline,
                           strategy, derivative, filename])
    ...
    for path_idx, s3_path in enumerate(s3_paths):
        ...
        try:
            if not os.path.exists(download_file):
                ...
                print('{0:.3f}% percent complete'.format(
                    100*(float(path_idx+1)/total_num_files)))
            else:
                print('File {0} already exists, skipping...'.format(
                    download_file))
        except Exception as exc:
            print('There was a problem downloading {0}.\n Check input arguments and try again.'.format(
                s3_path))
...
# Pipeline used to preprocess the data (e.g. 'cpac')
desired_pipeline = 'cpac'
# Noise-removal strategy used during preprocessing
desired_strategy = 'filt_noglobal'
# Path to local folder to download files to
download_data_dir = 'data/ABIDE'
# 'asd', 'tdc', or 'both' corresponding to the diagnosis of the participants for whom data should be downloaded
desired_diagnosis = 'both'

# Call the collect and download routine
collect_and_download(desired_derivative, desired_pipeline,
                     desired_strategy, download_data_dir, desired_diagnosis)
